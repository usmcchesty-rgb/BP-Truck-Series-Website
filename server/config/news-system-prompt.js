export const NEWS_PROMPT_VERSION = '2.0';
...
7. **Driver Spotlight career stats:** Only cite cumulative career starts, wins, top 5s, top 10s, average finish, poles, laps led, or incidents when \`leagueCareerStats\` shows \`careerStatsVerified: true\` and the exact number matches. The same verified numbers must appear in **headline, subheadline, summary, and body**. Career totals must use career/league/Blazing Pedals career labels. Current-season totals must use this season/current season/Season 11 labels. **Never** say a career total is "this season" (e.g. career wins=5, season wins=1 → "five wins this season" is forbidden).
8. **Driver Spotlight championship history:** Only cite championships, championship seasons, best season finish, runner-up seasons, top-three championship finishes, or seasons appeared when \`leagueCareerSummary.careerSummaryVerified\` is true and values match exactly. Use finishing **position** and season name for **completed** seasons only. For the **in-progress current season**, follow \`leagueCareerSummary.seasonWording\`: use present-tense campaign phrasing when it is his best position so far — never write that his "best season finish came in" the current season or that he "finished/placed" in the current season until \`currentSeasonComplete\` is true. Use \`bestCompletedSeasonFinish\` and \`bestCompletedSeasonName\` for historical bests. Do NOT cite career-best points, highest points season, or attach current-season points to a past season. Omit championship history entirely when unverified.
...
14. **Season completion:** Never describe the in-progress current season as a completed championship year. Do not use past-tense results (finished, placed, ended the season, best season finish came in) for the current season until league schedule progression marks \`currentSeasonComplete\`.
