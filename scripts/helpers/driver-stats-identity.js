import { slugify } from '../../api/_lib.js';
import { normalizeCustomerId } from '../../api/_driver-number-reservations.js';
import { normalizeDriverWriteName } from '../../api/_drivers-write-identity.js';

export function slugifyDriverStatsName(value) {
...
function collectProfileNames(profile = {}) {
  ...
  for (const field of [profile.iracing_name, profile.display_name, profile.driver_name]) {
    const normalized = normalizeDriverWriteName(field);
...
function standingsRowName(row = {}) {
  return normalizeDriverWriteName(row.driverName || row.driver);
}
...
export function buildStandingsIdentityLookupMaps(standingsRows = []) {
...
export function resolveDriverStatsIdentity(profile = {}, context = {}) {
  const profileDriverId = String(profile.driver_id || '').trim();
  const iracingCustomerId = normalizeCustomerId(profile.iracing_customer_id);
  const maps =
    context.standingsMaps || buildStandingsIdentityLookupMaps(context.standingsRows || []);
...
export function findStandingsRowForIdentity(identity, standingsRows = [], maps = null) {
...
export function countOfficialResultsForDriver(srhDriverId, schedules = {}) {
...
export function buildDriverStatsIdentityDiagnostics(profile = {}, context = {}) {
