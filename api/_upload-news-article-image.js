...
const STORAGE_PREFIX = "news/articles";
...
function publicStorageUrl(filename) {
  ...
  return `${base}/storage/v1/object/public/${STORAGE_BUCKET}/${filename.split("/").map(encodeURIComponent).join("/")}`;
}
...
async function removeExistingStorageObject(sb, filename) {
  ...
  if (error && !/not found|does not exist|object not found/i.test(error.message)) {
    throw new Error(`Could not remove existing article image: ${error.message}`);
  }
}
...
async function saveToSupabaseStorage(sb, uploadBuffer, storageKey) {
  ...
  return {
    ...
    featuredImageUrl: displayUrl,
    featuredImageUrlStored: storedUrl,
...
export async function uploadNewsArticleImage(body) {
  ...
  if (!isLocalDev()) {
    const err = new Error(
      "Supabase Storage is required to upload article images on the live site.",
    );
    err.status = 400;
    throw err;
  }
