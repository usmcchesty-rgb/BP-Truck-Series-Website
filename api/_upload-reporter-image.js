...
import {
  photoCacheVersion,
  resolveMilesApexImageDisplayUrl,
  stripPhotoUrlQuery,
  supabase,
  withPhotoCacheBust,
} from "./_lib.js";
...
const STORAGE_FILENAME = "miles-apex.png";
const LOCAL_REL_PATH = "assets/reporters/miles-apex.png";
...
async function removeExistingStorageObject(sb, filename) {
  ...
  if (error && !/not found|does not exist|object not found/i.test(error.message)) {
    throw new Error(`Could not remove existing reporter image: ${error.message}`);
  }
}

async function upsertReporterImageSettings(sb, storedUrl) {
  const updatedAt = new Date().toISOString();
  const patch = {
    id: 1,
    milesApexImageUrl: stripPhotoUrlQuery(storedUrl),
    milesApexImageUpdatedAt: updatedAt,
  };
  ...
  if (error) {
    throw new Error(`Could not save reporter image settings: ${error.message}`);
  }
...
async function saveToSupabaseStorage(sb, uploadBuffer) {
  ...
  const settings = await upsertReporterImageSettings(sb, storedUrl);
  const displayUrl = resolveMilesApexImageDisplayUrl(settings);

  return {
    filename: STORAGE_FILENAME,
    savedTo: `Supabase Storage (${STORAGE_BUCKET}/${STORAGE_FILENAME})`,
    milesApexImageUrl: displayUrl,
    milesApexImageUrlStored: storedUrl,
    milesApexImageUpdatedAt: settings.milesApexImageUpdatedAt,
    cacheVersion: photoCacheVersion(settings.milesApexImageUpdatedAt),
    storage: "supabase",
  };
}
...
export async function uploadReporterImage(body) {
  ...
  if (!isLocalDev()) {
    const err = new Error(
      "Supabase Storage is required to upload the reporter image on the live site.",
    );
    err.status = 400;
    throw err;
  }
